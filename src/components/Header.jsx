...
const Header = ({location}) => {
    ...
    return (
        <div className={`w-full flex justify-between items-center h-20 px-10 absolute ${isCarPage ? "" : "bg-white"} z-50 ${isAuthPage ? 'shadow-md' : ''}`}>
            <Link to={"/"} className='flex items-center gap-x-5 w-1/4'>
                <img src="/images/logo.png" className={`h-10 ${isCarPage ? "invert" : ""}`} alt="" />
                <span className={`font-bold text-xl ${isCarPage ? "text-white" : ""}`}>vovgoo dev.</span>
            </Link>
            <div className='w-2/4'>
                <ul className={`flex justify-center items-center font-bold gap-x-5 ${isCarPage ? "text-white" : ""}`}>
            ...
            <div className='flex w-1/4 justify-end'>
                <ul className={`flex items-center justify-center gap-x-5 ${isCarPage ? "text-white" : ""}`}>
                    {isAuthenticated && 
                    <li className='group'>
                    ...
                    <li className='group'>
                        <Link to={"/login"} className='flex items-center justify-center gap-x-3 cursor-pointer select-none'>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" className={`group-hover:stroke-amber-500 ${isCarPage ? "stroke-white" : "stroke-black"} duration-300 transition-all`} width="25px" height="25px" viewBox="0 0 24 24" fill="none">
